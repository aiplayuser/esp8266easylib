...
void homepage(){ String html = httphead() + "ID: " + espid + "-" + String(ESP.getFlashChipSize()/1024/1024) + "MB<p>" ;
    ...
    html+= "<form action=/upload name=form1 method=POST enctype=multipart/form-data><input type=file name=data onchange=document.form1.submit()></form><p>" ; //上传文件
    ...
    #ifdef ESP8266
        FSInfo fsinfo; SPIFFS.info(fsinfo); html+="FSInfo: "+String(fsinfo.usedBytes)+"/"+String(fsinfo.totalBytes)+"-Byte<p>"; //闪存信息
        Dir dir=SPIFFS.openDir("/"); while(dir.next()){ html+="<form action=/delone method=post><a href="+dir.fileName()+">"+dir.fileName()+"</a>--"+dir.fileSize();
                                                        html+="<input name=mingzi style=width:0px value="+dir.fileName()+"><input type=submit value=删除Del></form><p>";} 
    #elif defined(ESP32)
...
void again(String info = "R S T..."){ webserver.send( 200,"text/html",httphead()+"<h2>"+ info +"<br>稍后返回home</h2></body></html>" ); delay(2222); ESP.reset(); }
...
void loop1(){ if(!digitalRead(0)){ rjson("/config.json"); doc["pass"]=""; doc["ssid"]=espid; wjson("/config.json"); again(); } //监听板载按钮重置ssid和pass
     ...
     if(!mqttclient.connected())connectmqtt(); mqttclient.loop(); }  //监听mqtt服务
